...
const TeacherAssignmentSystem = () => {
	...
	const handleAssignTeacher = (courseId, teacherId, type, value) => {
		console.log("asssignments -> ", assignments);
		...
		if (existingAssignmentIndex > -1) {
			...
			// remove existing added load from the course (if there is)
			const oldLectLoad =
				updatedAssignments[existingAssignmentIndex].divisions *
				course.lectHrs;
			const oldLabLoad =
				updatedAssignments[existingAssignmentIndex].batches *
				course.labHrs;

			course.reqLectLoad += oldLectLoad;
			course.reqLabLoad += oldLabLoad;

			// update teacher assigned load
			teacher.assignedLoad -= oldLectLoad + oldLabLoad;
			...
			setTeachers((prevTeachers) =>
				prevTeachers.map((t) => (t._id === teacherId ? teacher : t))
			);

			// update courses
			setCourses((prevCourses) =>
				prevCourses.map((c) => (c._id === courseId ? course : c))
			);
		} else {
			console.log(courseId, teacherId, type, value);
			const newAssignment = {
				teacherId,
				courseId,
			...
			// add new load to the course
			course.reqLectLoad -= newAssignment.divisions * course.lectHrs;
			course.reqLabLoad -= newAssignment.batches * course.labHrs;

			// update teacher assigned load
			teacher.assignedLoad +=
				newAssignment.divisions * course.lectHrs +
				newAssignment.batches * course.labHrs;
				...
				<div className="absolute z-10 mt-1 w-64 bg-white rounded-md shadow-lg border border-blue-200 p-3">
					...
					<div className="flex items-center text-xs text-gray-600 mb-3">
						...
						<span className="ml-auto">
							Remaining: {getRemainingLoad(selectedTeacher._id)}{" "}
							hrs
						</span>
					</div>
